...
export function SubjectCard({
  ...
  const handleStartTest = () => {
    ...
    if (disciplineId) {
      ...
      const encodedTitle = translit(title);
      navigate(`/test/${encodedTitle}/${disciplineId}?type=learn`);
    } else {
      // Нет дисциплины: используем только название (менее надежно, но работает)
  ...
  // ========== ОБРАБОТЧИК ДЛЯ ПЕРЕХОДА К МАТЕРИАЛАМ ==========
  const handleUsefulMaterials = () => {
    handleClose();
    
    if (disciplineId) {
      // ✅ Используем только ID, без slug
      // const encodedTitle = encodeURIComponent(title);
      const encodedTitle = translit(title);
      navigate(`/usefulmaterials/${encodedTitle}/${disciplineId}`);
    } else {
      // Нет дисциплины: используем только название (менее надежно, но работает)
      console.warn(`⚠️ Нет disciplineId для предмета "${title}", используется только название`);
      const encodedTitle = encodeURIComponent(title);
      navigate(`/usefulmaterials/${encodedTitle}/?type=learn`);
    }
  };
// =======================================================
          ...
          <Box sx={{ 
            ...
            <Button 
              ...
              sx={{ 
                minWidth: { xs: "100%", sm: "150px" },
                py: { xs: 1, sm: 1.5 },
                backgroundColor: (theme) => theme.palette.custom?.quizLabel || "#FF8C00",
                '&:hover': {
                  backgroundColor: (theme) => theme.palette.custom?.videoLabel || "#CBB7FF",
                }
              }}
            >
              Корисні матеріали
