// //урощеный pdfExportUniversal.tsx без tableConfigs, getTableTypeById, detectTableTypeByData
...
//     try {
//       //console.log('📄 Генерація PDF...', data);
...
//       // console.log(`📄 Формат: ${pdfConfig.columns.length > 4 ? 'альбомний' : 'портретний'}`);
      
//       // Створення HTML
//       const element = createTableHTML(data, disciplineName, title, pdfConfig);
//       document.body.appendChild(element);

//       // ⏳ ДАЄМО ЧАС НА ЗАВАНТАЖЕННЯ ЗОБРАЖЕНЬ (2 секунди)
...
const createTableHTML = (
  ...
  data.forEach((row, idx) => {
    ...
    pdfConfig.columns.forEach(col => {
      ...
      // 🔥 ПРАВИЛЬНА ОБРОБКА РІЗНИХ ТИПІВ ЗНАЧЕНЬ
      let textValue = '';
      let isImage = false;
      let imageUrl = '';
      ...
      if (rawValue && typeof rawValue === 'object') {
        // Якщо це об'єкт з типом image (для майбутнього розширення)
      ...
      if (isImage && imageUrl) {
        ...
        if (textValue.length > 500) {
          textValue = textValue.substring(0, 497) + '...';
        }
