...
namespace TestMaturalnyApp.Services.Services.AI.Providers
{
    public class OpenAiProvider : IAiProvider
    {
            ...
            ILogger<OpenAiProvider> logger)
        {
            ...
            _logger = logger;
            _config = options.Value.Providers.GetValueOrDefault("OpenAI");
        }
        ...
        public async Task<string> GenerateExplanationAsync(string prompt, CancellationToken cancellationToken = default)
        {
            ...
            {
                _logger.LogWarning("OpenAI provider is not configured or disabled");
                return null;
            }
            ...
            try
            {
                ...
                {
                    _logger.LogWarning("OpenAI API error: {StatusCode}, {Error}", response.StatusCode, responseJson);
                    return null;
                }
            ...
            {
                _logger.LogError(ex, "OpenAI API exception");
                return null;
            }
        }
